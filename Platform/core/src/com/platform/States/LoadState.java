...
public class LoadState extends State {
    ...
    private boolean pressed, buttonPress, elementPressed, connectionPress, tablePress, reposition;
    ...
    private void checkButtons() {
        ...
        for(int i = 0; i < tables.size; ++i) {
            ...
            if(input.overlaps(t.getRectangle())) {
                ...
                dx = input.x - tables.peek().getX();
                dy = input.y - tables.peek().getY();
    ...
    private void input() {
        if(Gdx.input.isTouched()) {
            ...
            if(!tablePress && !reposition) checkButtons();

            if(tablePress && pressed) {
                tables.peek().setPosition(input.x - dx, input.y - dy);
            }
